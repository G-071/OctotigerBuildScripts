...
output="$(./build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger $octotiger_args p2p_kernel_type=old p2m_kernel_type=old m2m_kernel_type=old m2p_kernel_type=old)"
...
output="$(./build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger $octotiger_args p2p_kernel_type=old p2m_kernel_type=old m2m_kernel_type=soa_cpu m2p_kernel_type=old)"
running_time=$(echo "$output" | grep 'Computation' | sed 's/Computation: //g')
check_results $1 "$output"
if [ $errorcount -eq 0 ]; then
...
output="$(./build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger $octotiger_args p2p_kernel_type=old p2m_kernel_type=old m2m_kernel_type=old m2p_kernel_type=soa_cpu)"
running_time=$(echo "$output" | grep 'Computation' | sed 's/Computation: //g')
check_results $1 "$output"
if [ $errorcount -eq 0 ]; then
...
output="$(./build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger $octotiger_args p2p_kernel_type=soa_cpu p2m_kernel_type=old m2m_kernel_type=old m2p_kernel_type=old)"
running_time=$(echo "$output" | grep 'Computation' | sed 's/Computation: //g')
check_results $1 "$output"
if [ $errorcount -eq 0 ]; then
...
output="$(./build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger $octotiger_args p2p_kernel_type=old p2m_kernel_type=soa_cpu m2m_kernel_type=old m2p_kernel_type=old)"
running_time=$(echo "$output" | grep 'Computation' | sed 's/Computation: //g')
check_results $1 "$output"
if [ $errorcount -eq 0 ]; then
...
output="$(./build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger $octotiger_args p2p_kernel_type=soa_cpu p2m_kernel_type=soa_cpu m2m_kernel_type=soa_cpu m2p_kernel_type=soa_cpu)"
running_time=$(echo "$output" | grep 'Computation' | sed 's/Computation: //g')
check_results $1 "$output"
if [ $errorcount -eq 0 ]; then
