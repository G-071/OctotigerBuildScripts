...
for i in `seq $2 $3 $4`; do
	...
	output1="$(./../build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger -Ihpx.stacks.use_guard_pages=0 -t$i $octotiger_args p2p_kernel_type=old p2m_kernel_type=old m2m_kernel_type=old m2p_kernel_type=old)"
	check_results $1 "$output1"
	echo "Running test $i,2 - m2m on..." | tee -a LOG.txt
	output2="$(./../build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger -Ihpx.stacks.use_guard_pages=0 -t$i $octotiger_args p2p_kernel_type=old p2m_kernel_type=old m2m_kernel_type=soa_cpu m2p_kernel_type=old)"
	check_results $1 "$output2"
	echo "Running test $i.3 - m2p on..." | tee -a LOG.txt
	output3="$(./../build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger -Ihpx.stacks.use_guard_pages=0 -t$i $octotiger_args p2p_kernel_type=old p2m_kernel_type=old m2m_kernel_type=old m2p_kernel_type=soa_cpu)"
	check_results $1 "$output3"
	echo "Running test $i.4 - p2p on..." | tee -a LOG.txt
	output4="$(./../build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger -Ihpx.stacks.use_guard_pages=0 -t$i $octotiger_args p2p_kernel_type=soa_cpu p2m_kernel_type=old m2m_kernel_type=old m2p_kernel_type=old)"
	check_results $1 "$output4"
	echo "Running test $i.5 - p2m on..." | tee -a LOG.txt
	output5="$(./../build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger -Ihpx.stacks.use_guard_pages=0 -t$i $octotiger_args p2p_kernel_type=old p2m_kernel_type=soa_cpu m2m_kernel_type=old m2p_kernel_type=old)"
	check_results $1 "$output5"
	echo "Running test $i.6 - All on..." | tee -a LOG.txt
	output6="$(./../build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger -Ihpx.stacks.use_guard_pages=0 -t$i $octotiger_args p2p_kernel_type=soa_cpu p2m_kernel_type=soa_cpu m2m_kernel_type=soa_cpu m2p_kernel_type=soa_cpu)"
	check_results $1 "$output6"
	echo "Running test $i.7 - All on except p2m..." | tee -a LOG.txt
	output7="$(./../build-cpu-RelWithDebInfo-jemalloc/octotiger/octotiger -Ihpx.stacks.use_guard_pages=0 -t$i $octotiger_args p2p_kernel_type=soa_cpu p2m_kernel_type=old m2m_kernel_type=soa_cpu m2p_kernel_type=soa_cpu)"
